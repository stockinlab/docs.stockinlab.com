...
import type {DefaultThemeData, DefaultThemeNormalPageFrontmatter, ResolvedSidebarItem, SidebarConfigArray, SidebarConfigObject, SidebarItem,} from '../../shared';
 ...
 */
export const headerToSidebarItem = (header: PageHeader, sidebarDepth: number): ResolvedSidebarItem => ({
    text: header.title,
    link: `#${header.slug}`,
    children: headersToSidebarItemChildren(header.children, sidebarDepth),
});

export const headersToSidebarItemChildren = (headers: PageHeader[], sidebarDepth: number): ResolvedSidebarItem[] =>
    sidebarDepth > 0
 ...
 */
export const resolveAutoSidebarItems = (sidebarDepth: number): ResolvedSidebarItem[] => {
 ...
 */
export const resolveArraySidebarItems = (sidebarConfig: SidebarConfigArray, sidebarDepth: number): ResolvedSidebarItem[] => {
    const route = useRoute();
    const page = usePageData();

    const handleChildItem = (item: ResolvedSidebarItem | SidebarItem | string): ResolvedSidebarItem => {
 ...
 */
export const resolveMultiSidebarItems = (sidebarConfig: SidebarConfigObject, sidebarDepth: number): ResolvedSidebarItem[] => {
    const route = useRoute();
